...
public final class ConversionUtils implements Serializable {
   ...
   * @return List of Iterable elements. If given <code>iterable</code> is <code>null</code>, an
   ...
   * @param iterable Iterable to convert
   * @return Set of Iterable elements. If given <code>iterable</code> is <code>null</code>, an empty
   *     set is returned
   */
  ...
  /**
   * Try to convert given <code>value</code> to required Enum type, using ordinal or constant name
  ...
  public static <T extends Enum<T>> T convertEnumValue(Class<T> type, Object value)
      ...
      if (TypeUtils.isEnum(value.getClass())) {
        ...
        if (ordinal == null || ordinal.intValue() >= enmValues.length) {
          throw new IllegalArgumentException(
              "Unable to convert to required Enum type "
                  + type
                  + ": ordinal value "
                  + ordinal
                  + " is out of range");
   ...
   *
   * <p>Supported types are: Characters, Booleans, Numbers and Enums.
   *
   ...
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
  public static <T> T convertStringValue(String str, Class<T> type)
      throws IllegalArgumentException {
    if (str != null && str.length() > 0) {
      ...
      if (TypeUtils.isEnum(type)) {
        ...
        if (enumType == null) {
          throw new IllegalArgumentException(
              "The target type " + type + " does not refer to an enum");
        }
  ...
  /**
   * Parse the given String value into a {@link Number} instance of the given target class. Supports
   * numbers in hex format (with leading "0x", "0X", or "#") as well.
   *
   * @param <T> Number type
  ...
  public static <T extends Number> T parseNumber(String value, Class<T> targetClass) {
    ...
    if (TypeUtils.isByte(targetClass)) {
      ...
      return (T)
          (FormatUtils.isHexNumber(trimmed) ? Short.decode(trimmed) : Short.valueOf(trimmed));
    } else if (TypeUtils.isInteger(targetClass)) {
      return (T)
          (FormatUtils.isHexNumber(trimmed) ? Integer.decode(trimmed) : Integer.valueOf(trimmed));
      ...
      return (T)
          (FormatUtils.isHexNumber(trimmed) ? decodeBigInteger(trimmed) : new BigInteger(trimmed));
  ...
  public static <T extends Number> T convertNumberToTargetClass(Number number, Class<T> targetClass)
    ...
    if (targetClass.isInstance(number)) {
      ...
      if (bigInt != null
          && (bigInt.longValue() < Long.MIN_VALUE || bigInt.longValue() > Long.MAX_VALUE)) {
        throw new IllegalArgumentException(
            "Could not convert number ["
                + number
                + "] of type ["
   ...
   * @return Converted {@link LocalDate}, or <code>null</code> if given calendar was <code>null
   *     </code>
   */
  ...
  /**
   * Convert a {@link Date} value into a {@link LocalDate}, using default calendar and default time
   * zone.
   *
  ...
  /**
   * Convert a {@link LocalDate} value into a {@link Date}, using default calendar and default time
   * zone.
   *
   * @param date Date to convert (may be null)
   ...
   *
   * @param calendar Calendar to convert (may be null)
   * @return Converted {@link LocalDateTime}, or <code>null</code> if given calendar was <code>null
   *     </code>
   */
  ...
  /**
   * Convert a {@link Date} value into a {@link LocalDateTime}, using default calendar and default
   * time zone.
   *
   * @param dateTime Date to convert (may be null)
   * @return Converted {@link LocalDateTime}, or <code>null</code> if given date was <code>null
   *     </code>
   */
  ...
  /**
   * Convert a {@link LocalDateTime} value into a {@link Date}, using default calendar and default
   * time zone.
   *
  ...
  /**
   * Convert a {@link Date} value into a {@link LocalTime}, using default calendar and default time
   * zone.
   *
   * @param date Date to convert (may be null)
  ...
  public static <T> T convert(Object value, Class<T> targetType) throws TypeMismatchException {
    Assert.notNull(targetType, "Target type must be not null");
    if (value != null) {
      ...
      if (TypeUtils.isNumber(targetType) && TypeUtils.isNumber(value.getClass())) {
        return (T)
            ConversionUtils.convertNumberToTargetClass((Number) value, (Class<Number>) targetType);
      }
