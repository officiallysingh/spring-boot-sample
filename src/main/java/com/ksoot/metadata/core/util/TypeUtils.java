...
public final class TypeUtils implements Serializable {
   ...
   *
   * @param type Type to check
   * @return <code>true</code> if it is a {@link LocalDate}, a {@link LocalTime} or a {@link
   *     LocalDateTime}
   */
   ...
   * @return If type is a primitive type, the corresponding object wrapper type is returned.
   *     Otherwise, returns the given type.
   */
  @SuppressWarnings("unchecked")
  public static <T> Class<T> box(Class<T> type) {
    Assert.notNull(type, "Type to unbox must be not null");
